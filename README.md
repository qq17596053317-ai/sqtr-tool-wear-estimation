...
Alternatively, set the environment variable `SQTR_RAW_DATA_DIR` to the folder
containing the two external high-resolution CSV bundles before running the
randomised degradation analysis.
...
5. `step22_tree_ensemble_baselines.m` through
   `step32_high_wear_underestimation_audit.m`; the deployable-response analysis
   is generated by `step28_randomized_fault_repeats.m`
...
8. `verify_archived_results.m`, `audit_v17_results.m` and
   `verify_deployable_baselines.m`
9. `figures/generate_v17_figure2_3.py` and
   `figures/generate_deployable_baseline_figure.py`; Supplementary Figure S3 is
   regenerated by `figures/generate_supplementary_figure_s3.py`
...
- Detector-guided median replacement: replace the flagged channel-feature block
  by medians estimated from the corresponding outer-training units, then apply
  the frozen all-channel ridge model
- Mask-aware sensor-dropout ridge: retain every clean training record, append a
  three-channel binary mask, and add one randomly single-channel-masked copy
  with probability in `{0.25,0.50,1.00}`; dropout probability and ridge lambda
  are selected jointly by inner grouped validation
...
- `additional_validation_results/reviewer_improvements`: 30-repeat randomised
  artificial degradation, deployable robust-baseline comparisons and
  reviewer-requested checks
- `figures/deployable_baselines`: Figure 5 source data and 600-dpi/vector
  exports for the deployable-baseline comparison
