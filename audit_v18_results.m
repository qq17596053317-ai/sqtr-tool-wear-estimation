...
figure2Cases.ComparatorMinusSQTR_MAE_mm = ...
    figure2Cases.FixedAllSensorPlusTimeMAE_mm - figure2Cases.SQTR_MAE_mm;
...
result(4) = "Input-matched case-macro difference (comparator - SQTR)";
value(4) = sprintf("%.6f mm; 95%% CI [%.6f, %.6f]; Holm p=%.6f", ...
    -route.MacroMAEDifferenceBMinusA,-route.MacroDifferenceCIUpper, ...
    -route.MacroDifferenceCILower,route.HolmP);
...
result(7) = "Positive-bias fixed-grid relative MAE (fixed, automatic, Oracle)";
...
fprintf("V18 audit passed. Outputs: %s\n",outputFolder);
